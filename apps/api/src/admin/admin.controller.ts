...
export class AdminController {
  ...
  @ApiOperation({
    summary: "Promote/demote user to a specific role (ADMIN only)",
  })
  promoteUser(
    @Param("id") id: string,
  ...
  @ApiOperation({
    summary: "Suspend a user (sets role to VIEWER) (ADMIN only)",
  })
  suspendUser(
    @Param("id") id: string,
    @Request() req: { user: { sub: string } },
  ...
  @Delete("users/:id")
  @ApiOperation({
    summary: "Delete a user account (ADMIN only) — irreversible",
  })
  deleteUser(
    @Param("id") id: string,
    @Request() req: { user: { sub: string } },
  ...
  @ApiOperation({
    summary:
      "Seed demo data for sales demos (ADMIN only, no-op if data exists)",
  })
  seedDemo(@Request() req: { user: { sub: string } }) {
    return this.admin.seedDemoData(req.user.sub);
  }
}
