...
export class CampaignsService {
  ...
  async create(dto: CreateCampaignDto) {
    ...
    void this.webhooks.dispatch('campaign.created', { id: campaign.id, name: campaign.name, status: campaign.status });
    return campaign;
  }
  ...
  async update(id: string, dto: UpdateCampaignDto) {
    ...
    void this.webhooks.dispatch('campaign.updated', { id: updated.id, name: updated.name, status: updated.status });
    return updated;
  }
   ...
   */
  async createVariant(campaignId: string, parentLinkId: string, label: string, destinationUrl?: string) {
    const parent = await this.prisma.trackingLink.findUnique({ where: { id: parentLinkId } });
    ...
    const abGroupId = (parentMeta['abGroupId'] as string | undefined) ?? parent.id;

    // Ensure parent is tagged as group root if not yet
    if (!parentMeta['abGroupId']) {
      await this.prisma.trackingLink.update({
        where: { id: parent.id },
        data: { metadata: { ...parentMeta, abGroupId, variantLabel: 'A', isAbParent: true } },
      });
    }
  ...
  async getVariants(groupId: string) {
    ...
    return links.map((l) => {
      ...
      return {
        ...
        conversionRate: l.clickCount > 0 ? (l._count.touchpoints / l.clickCount) * 100 : 0,
