...
export class ShopifyWebhookProcessor extends WorkerHost {
  ...
  async process(job: Job<ShopifyWebhookJobData>): Promise<void> {
    ...
    if (topic === 'orders/paid') {
      await this.shopify.handleOrderPaid(shopDomain, payload as unknown as Parameters<ShopifyService['handleOrderPaid']>[1]);
      this.logger.log(`Job ${job.id} complete: order processed`);
      return;
    }

    if (topic === 'orders/cancelled') {
      await this.shopify.handleOrderCancelled(shopDomain, payload as unknown as Parameters<ShopifyService['handleOrderCancelled']>[1]);
